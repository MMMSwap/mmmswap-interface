...
export function useDerivedMintInfo(
  ...
  const [, pairWithDummy] = usePair(currencies[Field.CURRENCY_A], currencies[Field.CURRENCY_B])
  ...
  const newPrice = useMemo(() => {
    ...
    const newPair = new Pair(
      new TokenAmount(pairWithDummy.reserve0.token, JSBI.add(pairWithDummy.reserve0.raw, currencyAAmount.raw)),
      new TokenAmount(pairWithDummy.reserve1.token, JSBI.add(pairWithDummy.reserve1.raw, currencyBAmount.raw))
    )
    const wrappedCurrencyA = wrappedCurrency(currencyA, chainId)
  ...
  const liquidityMinted = useMemo(() => {
    ...
    if (pair && totalSupply && tokenAmountA && tokenAmountB && Number(currencyAAmount?.toSignificant(6)) > 0 && Number(currencyBAmount?.toSignificant(6)) > 0) {
