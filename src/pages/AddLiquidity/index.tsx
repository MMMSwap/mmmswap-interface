...
import { Currency, currencyEquals, ETHER, TokenAmount, WETH } from 'goswap-sdk'
...
export default function AddLiquidity({
  ...
  const [approvalA, approveACallback] = useApproveCallback(parsedAmounts[Field.CURRENCY_A], router)
  const [approvalB, approveBCallback] = useApproveCallback(parsedAmounts[Field.CURRENCY_B], router)
  ...
  const handleCurrencyBSelect = useCallback(
    (currencyB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${currencyIdA ? currencyIdA : 'CURRENCY'}/${newCurrencyIdB}`)
      }
    },
            ...
            {noLiquidity ||
              (isCreate && (
                <ColumnCenter>
                  <BlueCard>
                    <AutoColumn gap="10px">
                      <TYPE.link fontWeight={600} color={'#6da8ff'}>
                        {t('you-are-the-first-liquidity-provider')}
                      </TYPE.link>
                      <TYPE.link fontWeight={400} color={'#6da8ff'}>
            ...
            {!account ? (
              ...
              <AutoColumn gap={'md'}>
                ...
                <ButtonError
                  ...
                  disabled={!isValid || approvalA !== ApprovalState.APPROVED || approvalB !== ApprovalState.APPROVED || Number(parsedAmounts[Field.CURRENCY_A]?.toSignificant(6)) === 0 || Number(parsedAmounts[Field.CURRENCY_B]?.toSignificant(6)) === 0}
